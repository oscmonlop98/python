...
def sendGift(*args):
    ...
    if len(idxs)==1:
        ...
        sentmsg.set("Sending %s to leader of %s" % (gifts[gift.get()], name))
        p.start(55)
...
frame = ttk.Frame(c)
frame.grid(column=2, row=0, sticky=(N,W,E,S))
...
lbl = ttk.Label(frame, text="Send to country's leader:")
g1 = ttk.Radiobutton(frame, text=gifts['card'], variable=gift, value='card')
g2 = ttk.Radiobutton(frame, text=gifts['flowers'], variable=gift, value='flowers')
g3 = ttk.Radiobutton(frame, text=gifts['nastygram'], variable=gift, value='nastygram')
send = ttk.Button(frame, text='Send Gift', command=sendGift, default='active')
sentlbl = ttk.Label(frame, textvariable=sentmsg, anchor='center')
...
ttk.Sizegrip(c).grid(column=1, row=1, sticky=(S,E))


p = ttk.Progressbar(c, orient=HORIZONTAL, length=200, mode='determinate')
