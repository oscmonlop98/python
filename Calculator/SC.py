...
# VIEW
def key(event):
    return(fsm.chState(event.char))


def buildButton(frame, key, r, c):
    myButton = ttk.Button(frame)
    #myButton['font'] = ('verdana', 12)
    #myButton['padx'] = 10
    #myButton['pady'] = 1
    myButton['width'] =10
    myButton['padding'] =(10,10,10,10)
    #myButton['height'] =2
    myButton['text'] = key
    myButton['command'] = lambda: fsm.chState(key)
    myButton.grid(row=r, column=c, sticky="we")
